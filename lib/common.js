...
function diff(from, to, key) {
	assert.arrayOfObject(from);
	assert.arrayOfObject(to);
	var n1, n2, i, j, found;
	var add = {};
	var del = {};
	var diffs = {
	...
	for (i = 0; i < from.length; i++) {
		n1 = from[i];
		found = false;
		for (j = 0; !found && j < to.length; j++) {
			n2 = to[j];
			if (jsprim.deepEqual(n1, n2))
				found = true;
		}
		if (j === to.length && !found) {
			if (key)
				del[n1[key]] = n1;
			else
				diffs.del.push(n1);
	...
	for (i = 0; i < to.length; i++) {
		n1 = to[i];
		found = false;
		for (j = 0; !found && j < from.length; j++) {
			n2 = from[j];
			if (jsprim.deepEqual(n1, n2))
				found = true;
		}
		if (j === from.length && !found) {
			if (key)
				add[n1[key]] = n1;
			else
				diffs.add.push(n1);
	...
	Object.keys(add).forEach(function (id) {
		if (Object.keys(del).indexOf(id) !== -1) {
			diffs.mod.push(add[id]);
			diffs.total++;
			delete add[id];
			delete del[id];
		} else {
			diffs.add.push(add[id]);
			diffs.total++;
		}
	});
	Object.keys(del).forEach(function (id) {
		diffs.del.push(del[id]);
		diffs.total++;
		delete del[id];
	});
	/*
	 * XXX What if a new nic is added and primary is set to true?
	 * This means one of the UpdateNics is not necessary.
	 */
